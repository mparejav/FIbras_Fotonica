...
# Parámetros
Ref_idx = [1.744, 1.649, 1.00, 1.649, 1.744] # Indices de refracción
Radio = [62, np.inf, 101.20, -101.20, np.inf, -62.70]  # radios de curvatura
Distance = [1.11, 0.74, 2.78, 0.74, 1.11]  # distancias
Focos = []
...
M_LG1 = Lente_Gruesa(n_aire, Ref_idx[0], Ref_idx[1], Radio[0], Radio[1], Distance[0])

T_23 = Traslation(Distance[1], Ref_idx[1])

Ra_3 = Transmition(n_aire, Ref_idx[1], Radio[2])

T_34 = Traslation(Distance[2], n_aire)

Ra_4 = Transmition(Ref_idx[2], n_aire, Radio[3])

T_45 = Traslation(Distance[3], Ref_idx[3])

M_LG2 = Lente_Gruesa(Ref_idx[3], Ref_idx[4], n_aire, Radio[4], Radio[5], Distance[4])

# Definicion matriz 

M = M_LG2 @ T_45 @ Ra_4 @ T_34 @ Ra_3 @ T_23 @ M_LG1
