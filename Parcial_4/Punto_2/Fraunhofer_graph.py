...
def graph(mask, Intensity):
    ...
    size = mask.shape[0] # Da el numero de pixeles
    ...
    dx = physical_size_um / size  # Resolución espacial en um
    ...
    plt.subplot(1, 2, 1)
    plt.title('Mask')
    plt.imshow(mask, cmap='gray', extent=[x.min(), x.max(), y.min(), y.max()])
    ...
    X_screen = (λ * D_prima * fx) * 1e-3 # Convertir a mm
    Y_screen = (λ * D_prima * fy) * 1e-3 # Convertir a mm
...
def graph_1D_fraunhofer(mask, Intensity):
    """
    Grafica el perfil 1D central del patrón de difracción de Fraunhofer para una máscara dada.
    El corte se realiza a lo largo del eje x en el centro de la imagen.
    """
    size = mask.shape[0]
    dx = 1  # um
    ...
    # Vector de frecuencias espaciales (ciclos/um)
    fx = np.fft.fftshift(np.fft.fftfreq(size, d=dx))
    X_screen = (λ * D_prima * fx) * 1000  # mm

    # Perfil 1D: corte central en y
    perfil_1d = Intensity[size // 2, :]

    plt.figure(figsize=(8, 4))
    plt.plot(X_screen, perfil_1d / perfil_1d.max(), label="Fraunhofer 1D (borde)")
    plt.xlabel("x' [mm]")
    plt.ylabel("Intensidad Normalizada")
    plt.title("Perfil 1D del patrón de Fraunhofer (borde)")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()
...
#simulate_and_graph(shape='horizontal_slit', horizontal_slit_width_um = 1000)  # Ejemplo de uso con rendija horizontal

#simulate_and_graph(shape='image', image_path='/home/manuel/Documents/GitHub/FIbras_Fotonica/Parcial_4/Images/Edge.png')  # Ejemplo de uso con imagen

#simulate_and_graph(shape = 'cross_mask', L1 = 400, L2 = 400, h1 = 400, h2 = 400, t = 100, e = 100)  # Ejemplo de uso con cruz

#simulate_and_graph(shape='Edge')  # Ejemplo de uso con máscara de borde
